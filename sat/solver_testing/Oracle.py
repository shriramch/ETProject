...
def grep_result(stdout):
    ...
    for line in stdout.splitlines():
        if re.search("^(s )?UNSATISFIABLE$", line, flags=re.MULTILINE):
            result.append(SolverQueryResult.UNSAT)
        elif re.search("^(s )?SATISFIABLE$", line, flags=re.MULTILINE):
            result.append(SolverQueryResult.SAT)
        elif re.search("^(s )?UNKNOWN$", line, flags=re.MULTILINE):
